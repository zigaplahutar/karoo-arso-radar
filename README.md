...
ARSO ne objavlja georeference svoje radarske slike, zato je izračunana iz slike same:
iz nje je bilo prebranih 34 krajevnih oznak, jim pripisane koordinate in z robustnim
prilagajanjem izračunana linearna preslikava.

```
x = 153,2746 × zemljepisna dolžina − 1852,7596
y = −222,3361 × zemljepisna širina + 10589,8661
```

RMS 1,76 piksla, največje odstopanje 3,25 piksla. En piksel je približno 500 m, torej
je napaka pod kilometrom. Kontrola: radar Lisca, ki je na sliki označen s križcem, se
ujema na en piksel.

![Preverjanje kalibracije](docs/kalibracija.png)

Mreža je pravokotna in brez rotacije, zato zadostujeta dve premici. Koda preveri
razmerje stranic slike – če bi ARSO spremenil izsek, se marker rajši ne nariše, kot da
bi lagal. Vse skupaj je v `ArsoGeo.kt`.
...
- `RadarRepository.ZOOM_LEVELS` – stopnje povečave (privzeto 1, 2, 4, 8).
